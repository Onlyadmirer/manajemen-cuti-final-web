      ...
      <div class="flex justify-between items-center mb-6">
        <div class="bg-white dark:bg-gray-800 border-l-4 border-blue-500 p-4 rounded shadow-sm">
          <p class="font-bold text-gray-800 dark:text-white">
            Sisa Kuota Cuti Tahunan: <span class="text-blue-600 text-xl">{{ Auth::user()->annual_leave_quota }}</span>
            Hari
          </p>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                  ...
                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Periode
                  </th>
                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Durasi
                  </th>
                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Status
                  </th>
                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Aksi
                  </th>
                </tr>
              </thead>
                  ...
                  <tr>
                    ...
                    <td class="px-6 py-4 align-top">
                      @if ($leave->leave_type == 'annual')
                        <span
                          class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">Tahunan</span>
                      @else
                        <span
                    ...
                    <td class="px-6 py-4 whitespace-nowrap align-top">
                      ...
                      @if ($leave->status == 'rejected' && $leave->rejection_reason)
                        <div
                          class="p-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded text-xs text-red-600 dark:text-red-300 italic max-w-xs whitespace-normal">
                    ...
                    <td class="px-6 py-4 text-sm font-medium align-top">
                      @if ($leave->status == 'pending')
                        <form action="{{ route('leaves.destroy', $leave) }}" method="POST"
                          onsubmit="return confirm('Yakin ingin membatalkan cuti ini?');">
                          @csrf
                          @method('DELETE')
                          <button type="submit"
                            class="text-red-600 dark:text-red-400 hover:underline font-bold">Batalkan</button>
                        </form>
                      @elseif($leave->status == 'approved')
                        <a href="{{ route('leaves.download_pdf', $leave->id) }}" target="_blank"
