      ...
      <div class="flex">
        <div class="shrink-0 flex items-center">
          <a href="{{ route('dashboard') }}" class="flex items-center gap-3 group">
            <div
              class="h-10 w-10 bg-gradient-to-br from-blue-600 to-blue-700 rounded-xl p-2 shadow-md group-hover:shadow-lg transition-shadow">
              <x-application-logo class="block h-full w-full text-white" />
            </div>
            <div class="hidden md:block">
              <div class="text-lg font-bold text-gray-800 dark:text-gray-200 leading-tight">Manajemen Cuti</div>
              <div class="text-xs text-gray-500 dark:text-gray-400 leading-tight">Sistem Informasi</div>
            </div>
          </a>
        </div>
        ...
        <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
          ...
          @if (in_array(Auth::user()->role, ['division_manager', 'hr']))
            <x-nav-link :href="route('approvals.index')" :active="request()->routeIs('approvals.*')"
                ...
                if (Auth::user()->role == 'division_manager' && Auth::user()->managedDivision) {
                    $pendingCount = \App\Models\LeaveRequest::where('status', 'pending')
                        ->whereHas('user', function ($q) {
                            $q->where('division_id', Auth::user()->managedDivision->id)->where('role', 'employee');
                        })
                        ->count();
                }
                ...
                elseif (Auth::user()->role == 'hr') {
                    $pendingCount = \App\Models\LeaveRequest::where('status', 'approved_by_leader')
                        ->orWhere(function ($query) {
                            $query->where('status', 'pending')->whereHas('user', function ($q) {
              ...
              @if ($pendingCount > 0)
                <span
                  class="ml-2 bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full shadow-sm">{{ $pendingCount }}</span>
              @endif
            </x-nav-link>
          @endif
